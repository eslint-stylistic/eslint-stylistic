...
ruleTester.run('block-spacing', rule, {
  valid: [
    ...
    ...options.flatMap(option => typeDeclarations.flatMap(typeDec => emptyBlocks.map<ValidTestCase<['always' | 'never']>>(blockType => ({
  ...
  invalid: [
    ...options.flatMap(option => typeDeclarations.flatMap((typeDec) => {
    ...
    // With block comments
    ...options.flatMap(option => typeDeclarations.flatMap<InvalidBlockSpacingTestCase>((typeDec) => {
      const property
          = typeDec.nodeType === AST_NODE_TYPES.TSEnumDeclaration
            ? 'bar = 1'
            : 'bar: true;'
