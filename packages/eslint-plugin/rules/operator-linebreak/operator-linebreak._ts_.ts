...
export default createRule<RuleOptions, MessageIds>({
  ...
  create(context) {
     ...
     * @private
     */
    function validateNode(node: Tree.Node, rightSide: Tree.Node, operator: string): boolean {
      /**
    ...
    return {
      ...
      TSIntersectionType(node) {
        for (let i = 0; i < node.types.length - 1; i++) {
          validateNode(node.types[i], node.types[i + 1], '&')
        }
      },
      TSUnionType(node) {
        for (let i = 0; i < node.types.length - 1; i++) {
      ...
      TSConditionalType(node) {
        validateNode(node.extendsType, node.trueType, '?')
        validateNode(node.trueType, node.falseType, ':')
      },
    }
  },
})
