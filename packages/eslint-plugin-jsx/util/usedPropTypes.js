...
function inLifeCycleMethod(context, checkAsyncSafeLifeCycles) {
  ...
  while (scope) {
    if (scope.block && scope.block.parent && scope.block.parent.key) {
      ...
      if (LIFE_CYCLE_METHODS.includes(name))
        return true

      if (checkAsyncSafeLifeCycles && ASYNC_SAFE_LIFE_CYCLE_METHODS.includes(name))
        return true
    }
    scope = scope.upper
  }
...
function isNodeALifeCycleMethod(node, checkAsyncSafeLifeCycles) {
  ...
  if (LIFE_CYCLE_METHODS.includes(nodeKeyName))
    return true

  if (checkAsyncSafeLifeCycles && ASYNC_SAFE_LIFE_CYCLE_METHODS.includes(nodeKeyName))
    return true
...
module.exports = function usedPropTypesInstructions(context, components, utils) {
  ...
  return {
    ...
    'MemberExpression, OptionalMemberExpression': function (node) {
      if (isPropTypesUsageByMemberExpression(node, context, utils, checkAsyncSafeLifeCycles)) {
        markPropTypesAsUsed(node)
        return
      }
